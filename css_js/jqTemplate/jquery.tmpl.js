...
/*
    ...
    #模板|数据|填充状态（对象，删除模板）
    $(tmp).tmpl(dt).appendTo(box,true);
    
    #自动产生box，保留tmp,每次调用前都会清空box，可以用在需要刷新dt的地方
    $('#tmp').tmpl(dt).appendF(0);
    #自动产生box，生成完之后删除tmp
    $('#tmp').tmpl(dt).appendF(0,true);
    
    #在id为box的元素里填充dt，保留tmp
    $('#tmp').tmpl(dt).appendF('#box');
    #在id为box的元素里填充dt，生成完之后删除tmp
    $('#tmp').tmpl(dt).appendF('#box',true);
    
    #在box内后、内前、外前、外后进行填充,不删除模板 
    $('#tmp').tmpl(dt).appendTo\prependTo\insertBefore\insertAfter(0)
    #替换整个box，不会保留box标签，不删除模板
    $('#tmp').tmpl(dt).replaceAll(0)
    
    
    #自定义的temp对于部分元素的兼容性不佳（比如table），容易出现元素混乱，为确保万无一失应该使用script标签
    <temp></temp>
...
(function($, undefined) {
	...
	$.each({
		appendF: "append", //在内后加入,每次追加都会清空之前的内容，实现刷新
		prependTo: "prepend", //在内前加入
		appendTo: "append", //在内后加入
		insertBefore: "before", //在外前加入
		insertAfter: "after", //在外后加入
		replaceAll: "replaceWith", //原有的标签会消失，被整个替换
	}, function(name, original) {
		//		console.log(name + ":" +original)

		//填充对象，调用之后是否删除模板
		$.fn[name] = function(selector, delTemp) {
			if(selector == 0) {
				//如果box不存在，就自动生成
				if(obj.prev('box').length == 0)
					obj.before('<box></box>')
				//调用temp的前面的box标签
				selector = obj.prev('box');
			} else
				selector = $(selector);
			var ret = [],
				insert = selector,
			...
			/*
			 * 删除模板对象
			 * 开启之后模板调用一次就会被删除，无发再次调用该模板，也就无法更新
			 * 只适合调用一次的情况
			 * 
			 * 默认是不删除
			 * 
