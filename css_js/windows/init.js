...
myLib.desktop.win = {
	...
	newWin: function(options) {
		...
		var defaults = {
			...
			/* Posible are pixels or 'center' */
			WindowWidth: Math.round(wh['w'] * 0.6),
			/* Only pixels */
...
myLib.desktop.lrBar = {
	init: function() {
		//读取元素对象数据
		var myData = myLib.desktop.getMydata();
		var $default_tools = myData.panel.lrBar['default_tools'] //小图标
			,
			$def_tools_Btn = $default_tools.find('span') //小图标里的按钮
			,
			$default_app = myData.panel.lrBar['default_app'] //大图标
			,
			$def_app_Btn = $default_app.find('li') //大图标里的按钮
			,
			$start_btn = myData.panel.lrBar['start_btn'] //开始菜单入口
			,
			$start_item = myData.panel.lrBar['start_item'] //开始菜单内容
			,
		...
		//小图标点击
		$def_tools_Btn.click(function() {

			//改变部分按钮的点击效果
			if($(this).attr('id') == 'showZm_btn') {
				$('.windows').css('display', 'none')
				return
			} else if($(this).attr('id') == 'them_btn') {
				//				message('暂不支持该功能');
				//				return
			}
			...
			NewWindow(title, href, wid)
		})

		//大图标交互效果
		$def_app_Btn.hover(function() {
		...
		//单击大图标
		$def_app_Btn.click(function() {
				//改变部分按钮的点击效果
				if($(this).attr('id') == 'app3') {
		...
		//开始菜单内容点击
		$("#start_block li").click(function() {

			//改变部分按钮的点击效果
			if($(this).attr('id') == 'start0') {
				story()
				return
			}
			
			if($(this).attr('id') == 'start4') {
				//              message('暂不支持该功能');
				window.close();
				return
			}
	...
	init: function() {
		...
		var data = [
			...
			[{
				text: "Df主页",
				func: function() {
					NewWindow('', 'http://dfer.top?wap', 'start_btn')
				}
			}],		
			[{
				text: "关于",
				func: function() {
					NewWindow('about', '/homepage/home/js', 'start_btn')
...
//弹窗
function NewWindow(title, href, wid) {
