...
if(typeof define != 'undefined')
	...
	define([], function() {
		return {
			...
			init() {
				DfAttentionLog('🐟️️欢迎来到Df的站点🌧️', '50', '#ffffff');
				log(["QQ", ":", "3504725309"], ["red", "#000000", "blue"]);
				log("http://dfer.top", "");
				log("我有故事，也有酒，只不过故事太长，酒太浓", "red", 15);
			}
...
/*
 * 需要引入clipboard插件
 * <script src="/css_js/clipboard/clipboard.min.js"></script>
 * 复制字符串到剪贴板
...
function message(msg, color = 'white') {
	...
	$('body').append('<div class="publicColor ' + DivC + '" style="opacity: 0;position: fixed;right: 15%;top:0%;color:' + color + '"></div>');
	var obj = $("." + DivC);
	obj.html(msg);
	obj.animate({
		opacity: 1,
 ...
 */
function layerInit() {
	document.write("<script type='text/javascript' src='/css_js/layui/layer/layer.js'></script>"); //动态调用js文件
	$(function() {
...
function IsUseF12() {
	...
	if((window.console && (console.firebug || console.table && /firebug/i.test(console.table()))) || (typeof opera == 'object' && typeof opera.postError == 'function') || typeof console.profiles == "object") {
		return true;
	} else
		return false;
}
