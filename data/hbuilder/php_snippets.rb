...
#从这里开始自定义
  ...
  #  ==========  
    ...
    snippet 'df-showList' do |s|
    s.trigger = 'slist'
    s.expansion =  'show_list($1)'
  end
  
  snippet 'df-showFirst' do |s|
    s.trigger = 'sfirst'
    s.expansion =  'show_first($1)'
  end
  
  snippet 'df-update' do |s|
    s.trigger = 'update'
    s.expansion =  'update($1)'
  end
  
  snippet 'df-insert' do |s|
    s.trigger = 'insert'
    s.expansion =  'insert($1)'
  end
 ...
 * $0
 *
 * (c) Copyright 2021 dfer.top
 */'
  end
  
  
    command t(:quick_note) do |cmd|
    cmd.key_binding = 'CONTROL+I'
 ...
 */" }
    ...
    command t(:quick_notes) do |cmd|
    cmd.key_binding = 'CONTROL+SHIFT+I'
    cmd.output = :insert_as_snippet
    cmd.input = :none
    cmd.invoke { "
/*
