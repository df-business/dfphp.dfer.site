...
#  = df自定义,快捷键重名会导致失效 = 
#  ========== 
  snippet 'df-html' do |cmd|
    cmd.trigger = 'dfht'
    cmd.expansion = "<df-html>
    
$1</df-html>$0"
  end
  
    snippet 'df-header' do |cmd|
    cmd.trigger = 'dfh'
    cmd.expansion = "<df-header>
    
$1</df-header>$0"
  end
  
    snippet 'df-body' do |cmd|
    cmd.trigger = 'dfb'
    cmd.expansion = "<df-body>
    
$1</df-body>$0"
  end
  
    snippet 'df-footer' do |cmd|
    cmd.trigger = 'dff'
    cmd.expansion = "<df-footer>
    
$1</df-footer>$0"
  end
   snippet 'df-html-show' do |cmd|
    cmd.trigger = 'dfht'
    cmd.expansion = "<df-html/>$1"
  end
  
   snippet 'df-header-show' do |cmd|
    cmd.trigger = 'dfh'
    cmd.expansion = "<df-header/>$1"
  end
  
   snippet 'df-body-show' do |cmd|
    cmd.trigger = 'dfb'
    cmd.expansion = "<df-body/>$1"
  end
  
   snippet 'df-footer-show' do |cmd|
    cmd.trigger = 'dff'
    cmd.expansion = "<df-footer/>$1"
  end
   
   snippet 'df-if' do |cmd|
    cmd.trigger = 'dfi'
    cmd.expansion = "<df-if $1>
    
$0</df-if>"
  end
   
  snippet 'df-elif' do |cmd|
    cmd.trigger = 'dfei'
    cmd.expansion = "<df-elif $1>
    $0"
  end
   snippet 'df-else' do |cmd|
    cmd.trigger = 'dfe'
    cmd.expansion = "<df-else>
    $1"
  end
  
   snippet 'df-print' do |cmd|
    cmd.trigger = 'dfp'
    cmd.expansion = "<df-print val=\"$1\"/>$0"
  end
  snippet 'df-each' do |cmd|
    cmd.trigger = 'dfe'
    cmd.expansion = "<df-each $1>
    
$0</df-each>"
  end
   snippet 'df-val' do |cmd|
    cmd.trigger = 'dfv'
    cmd.expansion = "<df-val value=\"$1\"/>$0"
  end
  
   snippet 'df-each-cache' do |cmd|
    cmd.trigger = 'dfec'
    cmd.expansion = "<df-each-cache $1>
    
$0</df-each-cache>"
  end
   snippet 'df-val-cache' do |cmd|
    cmd.trigger = 'dfvc'
    cmd.expansion = "<df-val-cache value=\"$1\"/>$0"
  end
  
  snippet 'df-code' do |cmd|
    cmd.trigger = 'dfc'
    cmd.expansion = "<df-code>
...
#  = df自定义,快捷键重名会导致失效 = 
#  ==========   
  command t(:quick_br_htm) do |cmd|
    cmd.key_binding = 'CONTROL+ENTER'
    cmd.output = :insert_as_snippet
    cmd.input = :none
    cmd.invoke { "<br />" }
  end
   command t(:quick_note_htm) do |cmd|
    cmd.key_binding = 'CONTROL+N'
    cmd.output = :insert_as_snippet
    cmd.input = :none
    cmd.invoke { "<!--=========$1=========-->$0" }
  end
