...
//            'captcha'  => 'require',
        ...
        if ($result) {
            ...
            if (!dfer_captcha_check($data['captcha'], $captchaId, false)) {
                $this->error('图片验证码错误!');
            }
            ...
            if ($registerCaptcha == $data['captcha']) {
                dfer_captcha_check($data['captcha'], $captchaId, true);
        ...
        if (Validate::is($data['username'], 'email')) {
            $accountType = 'email';
        } else if (dfer_check_mobile($data['username'])) {
            $accountType = 'mobile';
        } else {
        ...
        $code = dfer_get_verification_code($data['username']);
        if (empty($code)) {
        ...
        if ($accountType == 'email') {
            ...
            $emailTemplate = dfer_get_option('email_template_verification_code');

            $user    = dfer_get_current_user();
            if ($user === false) {
            ...
            $result  = dfer_send_email($data['username'], $subject, $message);

            if (empty($result['error'])) {
                dfer_verification_code_log($data['username'], $code);
            ...
            dfer_verification_code_log($data['username'], $code, $expireTime);
