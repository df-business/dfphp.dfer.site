...
class LoginController extends HomeBaseController
{
    ...
    public function index()
    {
        ...
        if (dfer_is_user_login()) { //已经登录时直接跳到首页
    ...
    public function doLogin()
    {
        if ($this->request->isPost()) {
            ...
            if (!dfer_captcha_check($data['captcha'])) {
    ...
    public function passwordReset()
    {
        if ($this->request->isPost()) {
            ...
            if ($result) {
                ...
                if (!dfer_captcha_check($data['captcha'], $captchaId)) {
                    $this->error('验证码错误');
                }

                $errMsg = dfer_check_verification_code($data['username'], $data['verification_code']);
                if (!empty($errMsg)) {
                    $this->error($errMsg);
                }
            }
            ...
            switch ($log) {
                case 0:
                    $this->success('密码重置成功', dfer_url('user/Profile/center'));
                    break;
                case 1:
                    $this->error("您的账户尚未注册");
                    break;
