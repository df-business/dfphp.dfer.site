...
class UserModel extends Model
{
    ...
    public function doMobile($user)
    {
        ...
        if (!empty($result)) {
            $comparePasswordResult = dfer_compare_password($user['user_pass'], $result['user_pass']);
            $hookParam             = [
                'user'                    => $user,
            ...
            if ($comparePasswordResult) {
                ...
                $token = dfer_generate_user_token($result["id"], 'web');
                if (!empty($token)) {
                    session('token', $token);
                }
                return 0;
    ...
    public function register($user, $type)
    {
        ...
        if (empty($result)) {
            $data   = [
                ...
                'user_nickname'   => '',
                'user_pass'       => dfer_password($user['user_pass']),
                'last_login_ip'   => get_client_ip(0, true),
                'create_time'     => time(),
                'last_login_time' => time(),
            ...
            dfer_update_current_user($data);
            $token = dfer_generate_user_token($userId, 'web');
            if (!empty($token)) {
                session('token', $token);
            }
            return 0;
    ...
    public function emailPasswordReset($email, $password)
    {
        ...
        if (!empty($result)) {
            $data = [
                'user_pass' => dfer_password($password),
            ];
    ...
    public function editData($user,$field = [])
    {
        ...
        if (self::update($user,['id'=>$userId],$field)) {
            ...
            dfer_update_current_user($userInfo->toArray());
            return 1;
        }
        return 0;
    }
    ...
    public function editPassword($user)
    {
        ...
        if (!dfer_compare_password($user['old_password'], $pass['user_pass'])) {
            return 2;
        }
        $data['user_pass'] = dfer_password($user['password']);
    ...
    public function bindingMobile($user)
    {
        ...
        dfer_update_current_user($userInfo);
        return 0;
    }
