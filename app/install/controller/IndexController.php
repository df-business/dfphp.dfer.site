...
class IndexController extends BaseController
{
    protected function initialize()
    {
        if (dfer_is_installed()) {
            $this->error('网站已经安装', dfer_get_root() . '/');
        }

        if (!is_writable(DFER_DATA)) {
            echo '目录' . realpath(DFER_ROOT . 'data') . '无法写入！';
            abort(500, '目录' . realpath(DFER_ROOT . 'data') . '无法写入！');
        }
...
//        $safe_mode          = (ini_get('safe_mode') ? '<font color=red>[×]On</font>' : '<font color=green>[√]Off</font>');
        ...
        $folders    = [
            DFER_ROOT . 'api' . DIRECTORY_SEPARATOR,
            DFER_ROOT . 'app' . DIRECTORY_SEPARATOR,
            DFER_ROOT . 'data' . DIRECTORY_SEPARATOR,
    ...
    public function step4()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $sql  = dfer_split_sql(dirname(__DIR__) . '/data/thinkcmf.sql', $dbConfig['prefix'], $dbConfig['charset']);
            $apps = dfer_scan_dir(DFER_ROOT . 'app/*', GLOB_ONLYDIR);
            foreach ($apps as $app) {
                $appDbSqlFile = DFER_ROOT . "app/{$app}/data/{$app}.sql";
                if (file_exists($appDbSqlFile)) {
                    $sqlList = dfer_split_sql($appDbSqlFile, $dbConfig['prefix'], $dbConfig['charset']);
    ...
    public function setDbConfig()
    {
        ...
        $dbConfig['authcode'] = dfer_random_string(18);
    ...
    public function setSite()
    {
        ...
        $admin['id']            = 1;
        $admin['user_pass']     = dfer_password($admin['user_pass']);
        $admin['user_type']     = 1;
        ...
        try {
            dfer_set_option('site_info', $siteInfo);
    ...
    public function installTheme()
    {
        ...
        $result     = $themeModel->installTheme(config('template.dfer_default_theme'));
        if ($result === false) {
            $this->error('模板不存在!');
        }
