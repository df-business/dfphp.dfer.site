...
class ThemeController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $defaultTheme = config('template.dfer_default_theme');
        if ($temp = session('dfer_default_theme')) {
            $defaultTheme = $temp;
        }
    ...
    {
        $themesDirs = dfer_scan_dir(WEB_ROOT . "themes/*", GLOB_ONLYDIR);
    ...
    public function uninstall()
    {
        if ($this->request->isPost()) {
            ...
            if (config('template.dfer_default_theme') == $theme) {
    ...
    public function active()
    {
        if ($this->request->isPost()) {
            ...
            if ($theme == config('template.dfer_default_theme')) {
            ...
            $result = dfer_set_dynamic_config(['template' => ['dfer_default_theme' => $theme]]);

            if ($result === false) {
                $this->error('配置写入失败!');
            }
            session('dfer_default_theme', $theme);
            dfer_clear_cache();
    ...
    {
        $theme = $this->request->param('theme');
        cookie('dfer_design_theme', $theme, 3);
                ...
                foreach ($value as $varName => $varValue) {
                    if (isset($widget['vars'][$varName])) {
                        ...
                        if (in_array($widget['vars'][$varName]['type'], ['rich_text'])) {
                            $varValue = dfer_replace_content_file_url(htmlspecialchars_decode($varValue));
                        }
        ...
        foreach ($vars as $varName => $varValue) {
            if (isset($widgetInFile['vars'][$varName])) {
                ...
                if (in_array($widgetInFile['vars'][$varName]['type'], ['rich_text'])) {
                    $varValue = htmlspecialchars(dfer_replace_content_file_url(htmlspecialchars_decode($varValue), true));
