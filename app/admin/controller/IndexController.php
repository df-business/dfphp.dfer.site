...
class IndexController extends AdminBaseController
{
    ...
    public function index(AdminMenuService $service)
    {
        ...
        $menus          = cache('admin_menus_' . dfer_get_current_admin_id(), '', null, 'admin_menus');

        if (empty($menus)) {
            $menus = $adminMenuModel->menuTree();
            cache('admin_menus_' . dfer_get_current_admin_id(), $menus, null, 'admin_menus');
        }
        ...
        //$admin = Db::name("user")->where('id', dfer_get_current_admin_id())->find();
