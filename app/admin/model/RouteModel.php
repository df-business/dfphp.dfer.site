    ...
    public function getRoutes($refresh = false)
    {
        ...
        foreach ($routes as $er) {
            ...
            if (isset($info['scheme'])) { //插件
                $plugin     = dfer_parse_name($info['scheme']);
                $controller = dfer_parse_name($info['host']);
                ...
                $path = '\\dfer\\controller\\PluginController@index?' . http_build_query($pluginParams);
...
//            }
        ...
        if (strpos(dfer_version(), '5.') === 0) {
            if (strpos(dfer_version(), '5.0.') === false) {
                $routeDir = DFER_DATA . "route/"; // 5.1
            } else {
                $routeDir = DFER_DATA . "conf/"; // 5.0
            }

            $content = "<?php\treturn " . var_export($allRoutes, true) . ";";

        } else {
            $routeDir = DFER_DATA . "route/";
...
//                $routeCode .= "->name('$ruleName')";
    ...
    public function getAppUrls()
    {
        ...
        foreach ($apps as $app) {
            $urlConfigFile = dfer_get_app_config_file($app, 'url');
