...
class MenuLogic
{
    ...
    public static function importMenus($app)
    {
        ...
        $newMenus = [];
        if ($app == 'admin') {
            $filePatten         = DFER_ROOT . "vendor/thinkcmf/cmf-app/src/{$app}/controller/*Controller.php";
            $coreAppControllers = dfer_scan_dir($filePatten);

            $filePatten          = DFER_ROOT . "vendor/thinkcmf/cmf-appstore/src/controller/*Controller.php";
            $appStoreControllers = dfer_scan_dir($filePatten);

            $filePatten  = APP_PATH . $app . '/controller/*Controller.php';
            $controllers = dfer_scan_dir($filePatten);

            $controllers = array_merge($coreAppControllers, $appStoreControllers, $controllers);
        } else if ($app == 'user') {
            $filePatten         = DFER_ROOT . "vendor/thinkcmf/cmf-app/src/{$app}/controller/Admin*Controller.php";
            $coreAppControllers = dfer_scan_dir($filePatten);
