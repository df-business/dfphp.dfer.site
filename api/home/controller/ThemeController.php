    ...
    public function more()
    {
        ...
        if (!empty($oldMore['vars'])) {
            foreach ($oldMore['vars'] as $varName => $var) {
                switch ($var['type']) {
                    case 'image':
                        $vars[$varName] = dfer_get_image_url($var['value']);
                        break;
                    case 'file':
                        $vars[$varName] = dfer_get_file_download_url($var['value']);
                        break;
                    case 'array':
                        foreach ($var['value'] as $varKey => $varValue) {
                            ...
                            foreach ($varValue as $varValueKey => $varValueValue) {
                                switch ($var['item'][$varValueKey]['type']) {
                                    case 'image':
                                        $var['value'][$varKey][$varValueKey] = dfer_get_image_url($varValueValue);
                                        break;
                                    case 'file':
                                        $var['value'][$varKey][$varValueKey] = dfer_get_file_download_url($varValueValue);
                                        break;
                                    default:
            ...
            foreach ($oldMore['widgets'] as $widgetName => $widget) {
                ...
                if (!empty($widget['vars'])) {
                    foreach ($widget['vars'] as $varName => $var) {
                        ...
                        switch ($var['type']) {
                            case 'image':
                                $widgetVars[$varName] = dfer_get_image_url($var['value']);
                                break;
                            case 'file':
                                $widgetVars[$varName] = dfer_get_file_download_url($var['value']);
                                break;
                            case 'array':
                                foreach ($var['value'] as $varKey => $varValue) {
