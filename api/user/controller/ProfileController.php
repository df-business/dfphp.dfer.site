...
use dfer\controller\RestUserBaseController;
use think\Validate;
...
class ProfileController extends RestUserBaseController
{
    ...
    public function changePassword()
    {
        ...
        if (!dfer_compare_password($data['old_password'], $userPassword)) {
            $this->error('旧密码不正确!');
        }

        UserModel::where('id', $userId)->update(['user_pass' => dfer_password($data['password'])]);
    ...
    public function bindingEmail()
    {
        ...
        $errMsg = dfer_check_verification_code($data['email'], $data['verification_code']);
        if (!empty($errMsg)) {
            $this->error($errMsg);
        }
    ...
    public function bindingMobile()
    {
        ...
        if (!dfer_check_mobile($data['mobile'])) {
        ...
        $errMsg = dfer_check_verification_code($data['mobile'], $data['verification_code']);
        if (!empty($errMsg)) {
            $this->error($errMsg);
        }
