...
class CommentModel extends Model
{
    ...
    {
        return dfer_replace_content_file_url(htmlspecialchars_decode($value));
    }
    ...
    public function getMoreAttr($value)
    {
        ...
        if (!empty($more['thumbnail'])) {
            $more['thumbnail'] = dfer_get_image_url($more['thumbnail']);
        }

        if (!empty($more['photos'])) {
            foreach ($more['photos'] as $key => $value) {
                $more['photos'][$key]['url'] = dfer_get_image_url($value['url']);
            }
        }

        if (!empty($more['files'])) {
            foreach ($more['files'] as $key => $value) {
                $more['files'][$key]['url'] = dfer_get_image_url($value['url']);
            }
        }
        return $more;
    }
