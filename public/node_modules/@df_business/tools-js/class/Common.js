...
class Common {
     ...
     * @param {Boolean} isObj true 返回时间对象 false 返回时间字符串
     */
    datetimeNow(isObj) {
        var d = new Date();
        var timeN = {
    ...
    /**
     * 延迟半秒关闭当前qui弹窗
     * @param {Object} is_delay true 延迟 false 不延迟
     */
    closeW(is_delay) {
        if (is_delay)
    ...
    /**
     * 运行iframe里的函数
     * @param {Object} frameName 元素name
     * @param {Object} action 函数名
     */
    ...
    runParentFrameFunction(action) {
        // 子页面调用父页面的函数
        parent.window.action();
    }
    /**
     * 运行父级的父级iframe里的函数
     * @param {Object} action
     */
    runLastParentFrameFunction(action) {
        // 调用上上级页面的函数
        window.parent.parent.window.action();
    }
     ...
     * SplitStr(value, ",", -1);
     * @param {Object} str
     * @param {Object} split_str    分隔符
     * @param {Object} position 位置
     */
    splitStr(str, split_str, position) {
        let arr = str.split(split_str);
        if (position < 0)
     ...
     * 查找字符
     * @param {Object} str
     * @param {Object} find_str 查找的字符
     */
    findStr(str, find_str) {
     ...
     * @param {Object} split_char   分隔符
     * @param {Object} action   函数
     */
    strSplitRun(str, split_char, action) {
        data = str.split(split_char);
     ...
     * @param {Object} action   触发函数
     */
    tcChoice(content, action) {
        //不要用“action()”
        top.Dialog.confirm(content, action);
    }
     ...
     * @param {Object} action   函数
     * @param {Object} second   秒数
     * @param {Object} isLoop   是否循环
     */
    timer(action, second, isLoop) {
        var id;
        if (isLoop)
            // 在指定的间隔重复执行指定函数，不考虑函数是否执行完
            id = setInterval(action(), second * 1000);
        else
            // 在指定的间隔执行一次指定函数，会在函数执行完成之后继续下面的操作
            id = setTimeout(action(), second * 1000);
        return id;
    }
     ...
     * @param {Int} days 保存时间
     * @param {String} path 保存位置。/ 表示该 Cookie 在整个域名下的所有路径都可以访问
     */
    ...
    /**
     * 初始化数据库和表
     * const db = await df_tools_common.openDB('dfer.site','test');
     * @param {Object} dbName
    ...
    /**
     * 根据id设置表中数据
     * await df_tools_common.writeData(db, 1, {content: 'dfer.site'});
     * @param {Object} db
     * @param {Object} id 可以是数字或者字符串
     * @param {Object} data
     */
    async writeData(db, id, data) {
        return new Promise((resolve, reject) => {
    ...
    /**
     * 根据id读取表中数据
     * const data = await df_tools_common.readData(db, 1);
     * console.log(data);
     * @param {Object} db
     * @param {Object} id 可以是数字或者字符串
     */
    async readData(db, id) {
        return new Promise((resolve, reject) => {
            const storeName = db.objectStoreNames[0];
    ...
    /**
     * 根据id删除表中数据
     * await df_tools_common.deleteData(db, 1);
     * @param {Object} db
     * @param {Object} id 可以是数字或者字符串
     */
