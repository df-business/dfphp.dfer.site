...
jQuery.extend({
    ...
    message: function(msg, color = 'white') {
        ...
        $('body').append(
            `<div class="publicColor ${DivC}" style="z-index:99999;opacity: 0;position: fixed;right: 15%;top:0%;color:${color}"></div>`
        );
        var obj = $("." + DivC);
        obj.html(msg);
        obj.animate({
     ...
     */
    postAsync: function(url, data, action) {
        //默认调用普通异步
        var x = false;
        //详细异步
        if (x) {
     ...
     *
     */
    postLite: function(url, data, action, log = false) {
        //jquery写法
        $.post(url, data,
            function(x) {
                if (log)
     ...
     * @param {Object} action
     */
    postAllowOrigin: function(url, data, action) {
        $.ajax({
            url: url,
            type: 'GET',
            dataType: 'JSONP',
    ...
    floatQQ: function() {
        ...
        document.write(
            `<div class="qq_info"><a href='tencent://message/?uin=427729597&Site=&Menu=yes'><img src='${__BASE__}/images/float_QQ.jpg' /></a><!-- <a href='#'> <img src='${__BASE__}/images/float_WeChat.jpg' /></a>--></div>`
        );

        // 这一段表示的是 文字 和QQ ，如果你要添加N个QQ，就把上面的代码复制N次
        document.write("</div>");
        // 如果QQ在线的话，它会自动的变换成亮颜色。如果不在线它会自动变成灰色。这个可以不用控制它。所有的样式都在第一个“document.write”里面，可以在里面更改所有的样式。
            ...
            switch (Type) {
                case "input":
                    for (var i in Data) {
                        ...
                        } else if (el && el.attr('type') === 'checkbox') {
                            if (Data[i]) {
                        ...
                        } else if (el && el.attr('type') == undefined) {
                            el.val(Data[i]);
                            isexit = true;
                        }
                        if (!isexit) {
