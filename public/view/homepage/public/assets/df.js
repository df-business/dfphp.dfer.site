...
function float_QQ() {
    ...
    document.write("<div class=\"qq_info\"><a href='tencent://message/?uin=427729597&Site=&Menu=yes'><img src='" +
...
function date_list(v, x) {
    switch (x) {
        ...
        case "time":
            ...
            break;
        case "full":
            var xx = new Date(parseInt(v.replace("/Date(", "").replace(")/", ""), 10)).toLocaleDateString() + new Date(
                parseInt(v.replace("/Date(", "").replace(")/", ""), 10)).toLocaleTimeString();
            break;
        default:
            var xx = "error";
    }
...
function datetimeNow(isObj) {
    ...
    if (isObj)
        ...
        return timeN.Year + '-' + (timeN.Month < 10 ? "0" + timeN.Month : timeN.Month) + '-' + (timeN.Day < 10 ? "0" +
            timeN.Day : timeN.Day) + '-' + (timeN.Hour < 10 ? "0" + timeN.Hour : timeN.Hour) + '-' + (timeN.Minute <
            10 ? "0" + timeN.Minute : timeN.Minute) + '-' + (timeN.Second < 10 ? "0" + timeN.Second : timeN.Second)
}
            ...
            render: function(rowData, rowindex, value, column) {
                if (rowData.sh == 1)
                    ...
                    return '<div class="popupMenu_link arrow border white_bg" ridx = ' + rowindex +
                        ...
                        rowData.ID + '\',\'' + rowData.data + '\',\'' + rowData.q + '\',\'' + rowData.m +
                        '\',\'' + rowData.AllotCode + '\',\'' + rowData.OutShop + '\',\'' + rowData.InShop +
                        '\',\'' + rowData.Remark + '\');">通过</a></div>';
            }
        });
...
function DfAttentionLog(str, font_size, color) { //以特定的格式打印“Df提示”，使用“ Unicode_JS 编码”和“ Unicode_JS 解码”
    ...
    if (color == undefined)
        color = 'red'
    console.log("%c " + str, "font-size:" + font_size + "px;color:" + color + ";-webkit-text-fill-color:" + color +
