...
class DfTools {
    ...
    message(msg, color = 'white') {
        ...
        $('body').append('<div class="publicColor ' + DivC +
            '" style="z-index:99999;opacity: 0;position: fixed;right: 15%;top:0%;color:' + color + '"></div>');
        var obj = $("." + DivC);
        obj.html(msg);
        obj.animate({
            opacity: 1,
     ...
     *
     */
    post(url, data, action, log = false) {
        //jquery写法
        $.post(url, data,
            function(x) {
                if (log)
    ...
    */
    ...
    dfAttentionLog(str, font_size, color) { //以特定的格式打印“Df提示”，使用“ Unicode_JS 编码”和“ Unicode_JS 解码”
        if (str == undefined)
            str = 'Df提示'
        ...
        if (color == undefined)
            color = 'red'
        console.log("%c " + str, "font-size:" + font_size + "px;color:" + color + ";-webkit-text-fill-color:" +
            color +
    ...
    */
    runFrameFunctionById(id, action, x) {
    ...
    */
    ...
    getIframeEleByName(FrameName, ElementId, ElementAttr) {
    ...
    //获取框架上级页面元素的属性值
    getParentFrameEleById(ParentElementId, ElementAttr) {
    ...
    getIframeEleById(FrameId, ElementId, ElementAttr) {
        $('#' + FrameId).load(function() {
            return $("#" + FrameId).contents().find("#" + ElementId).attr(
                ElementAttr) //$("#" + FrameId).contents() 获取指定元素的内容
    ...
    */
    ...
    setEleHeight(FrameId, mainheight) {
    ...
    //获取某id下某一个元素,num从0开始
    finEleByIdAndNum(id, num) {
        return $('#' + id).children().eq(num);
    }

    //根据元素名字获取一个元素
    finEleByEleAndName(Ele, Name) {
        return $(Ele + "[name='" + Name + "']");
    }
    //输出被 jQuery 选择器匹配的元素的数量
    getEleCountByEle(Ele) {
    ...
    */
    markBdMap(x, y) { //BdMap(116.404, 39.915)     //根据经纬度在地图上标注一个位置
